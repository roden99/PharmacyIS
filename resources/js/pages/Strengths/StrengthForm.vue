...
const isFormValidated = () => {
    if (!form.strengthname.toString().trim()) {
...
const form = useForm({
    ...
    strengthname: props.strength?.strengthname || '',
...
<template>
    ...
    <FormCard :loading="isProcessing" size="md">
        <form @submit.prevent="Submit" class="space-y-4 mt-4">
            <BaseField legend="Strength Information" description="Enter strength details">
                <template #fields>
                    <FieldGroup>
                        <div class="grid w-full grid-cols-12 gap-4">
                            <Field class="col-span-12">
                                <FieldLabel class="font-normal">Strength Name:</FieldLabel>
                                <Input v-model="form.strengthname" required />
                            </Field>
                        </div>
                    </FieldGroup>
                </template>
